...
- Use these alias values when filtering tools like [`get_reviews`](/mcp-server/tools/reviews/get-reviews) by `sources`.
- For full source details (URL, status, ID), use [`get_all_aggregation_sources`](/mcp-server/tools/aggregation/get-all-aggregation-sources).
