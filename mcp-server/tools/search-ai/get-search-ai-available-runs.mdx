...
- Use run dates from this tool as `startDate`/`endDate` values when calling [`get_search_ai_citations`](/mcp-server/tools/search-ai/get-search-ai-citations), [`get_search_ai_accuracy_report`](/mcp-server/tools/search-ai/get-search-ai-accuracy-report), and other Search AI report tools.
