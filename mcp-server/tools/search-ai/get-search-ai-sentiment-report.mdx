...
| `businessNumbers` | list[integer] | Yes | — | Location IDs (from [`get_child_locations`](/mcp-server/tools/business/get-child-locations)) |
...
- `businessNumbers` is required — pass location IDs from [`get_child_locations`](/mcp-server/tools/business/get-child-locations).
- Use this alongside [`get_search_ai_citations`](/mcp-server/tools/search-ai/get-search-ai-citations) to understand both what AI engines say and where they got that information.
