...
| `businessNumbers` | list[integer] | Yes | — | Location IDs to include (from [`get_child_locations`](/mcp-server/tools/business/get-child-locations)) |
...
- Use [`get_search_ai_available_runs`](/mcp-server/tools/search-ai/get-search-ai-available-runs) to find valid `startDate`/`endDate` values.
