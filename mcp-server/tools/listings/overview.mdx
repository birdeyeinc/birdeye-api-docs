...
<CardGroup cols={1}>
  <Card title="get_listing" icon="address-card" href="/mcp-server/tools/listings/get-listing">
    Full listing profile for a single location — hours, categories, photos, per-site status
  </Card>
  <Card title="get_listing_location_status_report" icon="circle-check" href="/mcp-server/tools/listings/get-listing-location-status-report">
    Synced/Submitted/Action Required counts for all listing sites at a location
  </Card>
  <Card title="get_listing_insights" icon="chart-line" href="/mcp-server/tools/listings/get-listing-insights">
    Aggregated Google (and other) listing analytics — map views, calls, directions, website visits
  </Card>
  <Card title="get_listing_category_list" icon="tags" href="/mcp-server/tools/listings/get-listing-category-list">
    Available listing categories for a given site (Google, Bing, Facebook, etc.) and country
  </Card>
  <Card title="get_product_listing_list" icon="box" href="/mcp-server/tools/listings/get-product-listing-list">
...
<Warning>
  ...
  Call [`get_child_locations`](/mcp-server/tools/business/get-child-locations) first to get the list of valid location IDs.
</Warning>
