...
<CardGroup cols={1}>
  ...
  <Card title="edit_scheduled_social_post" icon="calendar-pen" href="/mcp/tools/social/edit-scheduled-social-post">
    Edit a scheduled post's text, media, schedule time or target locations before it goes out
  </Card>
...
- **Editing scheduled posts** (`edit_scheduled_social_post`): Google Business Profile, Facebook, Instagram, LinkedIn
...
- Pick the right edit tool: `edit_scheduled_social_post` for a post that has not gone out yet (text, media, schedule time and locations can all change), `edit_published_social_post` for one that is already live (text only). Check with `get_social_post` if you are unsure.
- `edit_scheduled_social_post` treats `subBusinessNumbers` as the complete list — locations left out are unscheduled from the post.
