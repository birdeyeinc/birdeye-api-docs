...
- **NPS here is review-derived.** `get_nps_over_time` and `get_nps_by_location` are computed from review data, served by the review service, and can be narrowed with `reviewSites`.
