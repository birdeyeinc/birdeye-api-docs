...
<CardGroup cols={1}>
  <Card title="get_reviews_nps_over_time" icon="arrow-trend-up" href="/mcp/tools/reports/get-reviews-nps-over-time">
    Track review-derived Net Promoter Score over time
  </Card>
  <Card title="get_reviews_nps_by_location" icon="ranking-star" href="/mcp/tools/reports/get-reviews-nps-by-location">
    Compare review-derived Net Promoter Score across locations
  </Card>
...
- **NPS here is review-derived.** `get_reviews_nps_over_time` and `get_reviews_nps_by_location` are computed from review data, served by the review service, and can be narrowed with `reviewSites`.
