...
<Info>
  Birdeye has two regional MCP endpoints. Use the one that matches your account's region:

  | Region | MCP Server URL |
  |--------|---------------|
  | Global (default) | `https://mcp.birdeye.com/mcp` |
  | EU | `https://mcp-eu.birdeye.com/mcp` |
...
<Steps>
  <Step title="Client discovers the server">
    Your AI client connects to your regional MCP endpoint (`https://mcp.birdeye.com/mcp` for Global, `https://mcp-eu.birdeye.com/mcp` for EU) and discovers the OAuth configuration automatically.
  </Step>
...
- Access tokens and refresh tokens expire in 7 days
- Tokens are scoped to the authenticated Birdeye business at issuance
- Deactivating a user account revokes their tokens

## Rate Limits

Birdeye enforces a rate limit of 50 requests per minute on the MCP Server to prevent excessive or abusive traffic from a single user's IP address and to ensure fair usage and service stability.
