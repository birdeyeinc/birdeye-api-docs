...
<Steps>
  ...
  <Step title="Dynamic client registration">
    The client registers itself with Birdeye's Auth0-backed identity provider. This happens automatically — no manual setup required.
  </Step>
  <Step title="Browser authorization">
    You are redirected to a Birdeye-branded login page to sign in and grant access. This only happens once.
  </Step>
...
The following AI clients are pre-authorized for OAuth redirects:
...
Other MCP clients with their own redirect URIs can still connect — they will be dynamically registered on first use.
...
- All tools are annotated as read-only (`readOnlyHint: true`) — the server never modifies your Birdeye data
- Sensitive fields such as reviewer email addresses are automatically filtered from responses
- OAuth state is encrypted at rest using Fernet symmetric encryption
